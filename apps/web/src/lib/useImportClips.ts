 ...
 *  - register the file in the source library (the Clips panel lists ALL of
 *    its ≤60s pieces, not just what's on the timeline),
 *  - place the first piece on the timeline, auto-trimmed to 60s (with a
 *    notice), respecting the store's 60s total guard.
 * Once a file is in the library its object URL belongs to the source (same
 * never-revoke rule as Clip.src), even when the timeline add is rejected.
 */
  ...
  const importFiles = async (files: FileList | File[]) => {
    ...
    try {
      for (const file of list) {
        ...
        addSource({ fileName: file.name, url: meta.url, duration: meta.duration });

        const overLimit = meta.duration > MAX_TIMELINE_DURATION;
        const outPoint = overLimit ? MAX_TIMELINE_DURATION : meta.duration;
        const pieceCount = Math.max(1, Math.ceil(meta.duration / MAX_TIMELINE_DURATION));
        ...
        if (!result.ok) {
          // The source stays in the library (which now owns the URL) — its
          // pieces can still be added once timeline space frees up.
        ...
        if (overLimit) {
          notify({
            type: 'info',
            message: `"${file.name}" is longer than 60s — added the first 60s. All ${pieceCount} pieces are in the Clips panel.`,
          });
        }
      }
    } finally {
